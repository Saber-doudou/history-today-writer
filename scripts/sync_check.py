...
# -*- coding: utf-8 -*-
"""
sync_check.py — 一键核验 history-today-writer 技能文件一致性（v9.8.0）

核对项：
  ① 规则数：writing_core.md + topics/* + archive/cold_rules.md 的规则编号并集
     vs rule_index.md 索引行数 vs SKILL.md 声称数（158 = 107 Rule + 51 Forbidden）
  ② Forbidden 数（51）
...
EXPECT_RULES = 107       # R1-R107
EXPECT_FORBIDDEN = 51    # F1-F51
EXPECT_TOTAL = 158       # 107 + 51
EXPECT_VERSION = "v9.8.0"  # SKILL.md 末尾 Version 行的期望版本号
...
def main() -> int:
    print("=" * 64)
    print("history-today-writer sync_check（v9.8.0）")
