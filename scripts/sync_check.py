...
# -*- coding: utf-8 -*-
"""
sync_check.py — 一键核验 history-today-writer 技能文件一致性（v9.8.5）

核对项：
  ① 规则数：writing_core.md + topics/* + archive/cold_rules.md 的规则编号并集
     vs rule_index.md 索引行数 vs SKILL.md 声称数（167 = 114 Rule + 53 Forbidden）
  ② Forbidden 数（53）
...
EXPECT_RULES = 114       # R1-R114
EXPECT_FORBIDDEN = 53    # F1-F53
EXPECT_TOTAL = 167       # 114 + 53
EXPECT_VERSION = "v9.8.5"  # SKILL.md 末尾 Version 行的期望版本号
...
def main() -> int:
    print("=" * 64)
    print("history-today-writer sync_check（v9.8.5）")
