...
| **Phase 2-3 写作** | `writing_core.md` + `rule_index.md` +（按题材）`topics/` 1 个 +（按需）`craft_optional.md` +（按需）`fact_checklist.md` | 核心：叙事结构 + 6维工具包 + 写作标准 + 89条hot规则（core 67 + 题材专项 22，sync_check ⑤ 实跑口径）+ 温控表；索引：199条规则编号+摘要；题材专项×3；非强制技法；写作侧事实核查清单 | 核心~85K + 索引18K + 题材8-16K（+按需12K+4K） |
...
| **Phase 3.6 判例** | `review/CASE_STUDIES.md` | 80条案例（续号至80；只Grep命中关键词，禁止整读进上下文） | ~131K |
...
- 写完后必须运行 `scripts/sync_check.py` 核验（规则数 197=142+55、版本号、文件路径、hot 规则正文完整性）
...
- **P2 不阻塞（2026-09-07，防 critic 空转）**：P2 级项不强制修复，记为 note；Reviewer 输出达标时**允许 pass=true 且 issues 为空**，禁止为凑条目虚构 P2（Snorkel Self-Critique Paradox：高基线任务上自批评易幻觉缺陷，98% 基线叠自评会掉到 57%）
...
| 写作核心 | `writing_core.md` | 叙事结构+6维工具包+89条hot规则（core 67 + 题材专项 22，sync_check ⑤ 实跑口径）+基础/hot禁止模式+温控表（v9.7.8瘦身：26条cold Forbidden正文与cold状态表已迁 archive） |
| 规则索引 | `rule_index.md` | 199条规则编号+摘要+温控+文件定位 |
...
| 判例库 | `review/CASE_STUDIES.md` | 80条案例（续号至80） |
...
*Version: v10.2.0 | 2026-09-24（吉法尔蒸汽飞艇首飞 L2：新增 Rule 144 科技工程「首次X」叙事补齐全弧 P1 cold（4/4 共识 v1 缺特拉普落点/方向/后续；199=144+55，hot 89 不变；归 CASE-80）；基于 v10.1.9 | 2026-09-23（海王星发现 L2：新增 Rule 143 AI点评事实纠错须独立信源核验后采纳 P0 hot（四AI投喂学习中任一AI指v1/v2事实错误须独立检索权威信源逐处核验，不得直接采信AI断言；ds称70角秒放大3倍应为20角秒、ima称亚当斯履历无据两处误纠经独立核验推翻，数字/原表述保留；198=143+55，hot 89=core 67+题材专项 22；R139 标题设问须主体明确 P1 由 hot 降级 cold 释放 hot 位，正文迁 archive/cold_rules.md；归 CASE-79 判例）；基于 v10.1.8 | 2026-09-18（纽约时报创刊 L2：新增 Rule 142 概念时序准确 P1 hot（ds 1/4 指出 v1 标题/正文「定下客观」绝对化；现代新闻客观性为20世纪职业规范，非1851创刊即定；197=142+55，hot 88→89=core 67+题材专项 22）；基于 v10.1.7 | 2026-09-16（蒙特利尔议定书签署 L2：新增 Rule 141 标题数量/面积类比须与正文同口径且标明时点 P1 hot（4/4 AI 共识：v1「北美洲那么大」与正文 2990万km² 口径不一、缺时点），196=141+55，hot 87→87=core 65+题材专项 22；R10/R22/R77/R56/R87 已覆盖多数差距不重复新增；归 CASE-75 判例）；基于 v10.1.6 | 2026-09-15（青霉素发现 L2：新增 Rule 139 标题设问须主体明确+反差具体 P1（Master 复审定调「标题太模糊…显得语文水平低下」，R133 细化，v2 改「发现青霉素的人，为什么没能把它变成药？」）+ Rule 140 抽象计量单位须补可感锚点 P2（ds/ima/豆包 3/4 指出「月产数百亿单位」无感）；193→195=139+55，hot 85→87=core 65+题材专项 22；「搁置/无人问津缺原因」经下沉测试 R14/R71 已覆盖不新增；「AI 优化版日期写死与权威源冲突（9月15日 vs 权威源9月3日）」归 CASE-74 判例）；基于 v10.1.5 | 2026-09-14（引力波首探 GW150914 L2：新增 Rule 137 科学/物理关键参数须用官方精确值禁区间化笼统 P1 + Rule 138 重大观测/实验发现须交代"为何可信"交叉验证机制 P1，均 4/4 共识；191→193=138+55，hot 83→85=core 63+题材专项 22；时间线重复/后续落点两项经下沉测试归 CASE 判例不入规则，CASE-72/73）；基于 v10.1.4 | 2026-09-11（Curator 棘轮 B 档：F23 自指残留合并删除（约束由 R14 P0 hot 承载，Forbidden 56→55，192→191=136+55，空洞 23/49/50 不回填）+ R92/R93/R100/R101/F52 五条 30 日零触发 hot→cold 降级试点（demoted_at=2026-09-11 保护期 30 天，正文逐字迁 archive/cold_rules.md，hot 87→83=core 61+题材专项 22）；依据 09-11 业界检索：重复残留安全删/题材专项高价值保留/5-8 条小批量试点观察 2 周，Master 授权 AI 圈定）；基于 v10.1.3 | 2026-09-11（Curator 棘轮首轮 A 档：F49/F50 自指残留三处清除——索引行+writing_core 正文行+温控键，约束并入 R94/R95，Forbidden 58→56；194→192=136+56；hot 87=core 64+题材专项 23 不变（F49/F50 本为无正文索引幽灵，不计入 hot 正文口径）；编号空洞 49/50 不回填，新增自 F59 续号，sync_check ② 已适配 KNOWN_FB_GAPS；出处=09-08 Curator 候选清单 dry-run，Master 授权周五执行）；基于 v10.1.2 | 2026-09-11（智利"9·11"阿连德殉职 L2：新增 Rule 136 选举上位类须点齐"得票率-过半与否-确认程序"三要素 P1（ds/千问/豆包 3/4 指出 v1"三成出头"模糊且漏 1970 国会确认程序）；193→194=136+58；Phase 3 下沉测试：死亡时刻凭戏剧感估写（正午前后 vs 实际下午近两点）、行动代号无据（"黎明行动"英文权威源查无）、设问标题题文呼应三项归 CASE 判例不入规则）；基于 v10.1.1 | 2026-09-10（收据留痕与一致性：write_ima_receipt 增 note_ids 数组，同日二次备份不再覆盖丢失旧 note_id（旧单值字段保留兼容）；sync_check 新增 ⑬ IMA 收据↔automation memory 一致性（基线 09-07 起，按日聚合同日多块，防双源漂移），检查项 22→23；根因：同日二次备份时收据 note_id 被覆盖而 automation memory 字段级补全不覆盖，审计链断裂且无校验）；基于 v10.1.0 | 2026-09-10（记忆分片维护机制：新增 scripts/sync_topics.py（check/sync 双模式，从归档/收据/日志权威源重建 exec_log 与 publish_history 行，幂等不覆盖人工行）+ sync_check 新增 ⑫ 记忆分片新鲜度硬校验（缺行即 ❌）+ L3 Phase 7b 后自动调用同步；修复 09-08 至 09-10 分片切出即冻结（publish_history 补 09-07 至 09-10 共 4 行、exec_log 补 09-08 至 09-10 共 3 行）；规则数不变 193=135+58）；基于 v10.0.3 | 2026-09-10（欧佩克成立 L2：新增 Rule 134 会议/成立类事件须点齐牵头方与东道主 P1（ds/ima 指出 v1 漏伊拉克卡塞姆+沙伊巴尼、阿方索头衔纠正）+ Rule 135 前因链须交代"首次未果再遭重击"递进 P1（4/4 AI 指出 v1 漏 1959 首次压价+马蒂协定），191→193=135+58；Phase 3 下沉测试：会议日期区间/数据锚点两项判机械类归 L0 校验）；基于 v10.0.2 | 2026-09-09（美国国名命名 L2：新增 Rule 133 标题优先读者自然疑问型 P1 + Forbidden #58 现代名词时代错置 P1；189→191=133+58）；基于 v10.0.1 | 2026-09-08（意大利停战 L2：新增 Rule 132 量词限定 P1 + Forbidden #57 同字重复 P2；Rule 36 细化占领区力量分布；187→189=132+57）；基于 v10.0.0 | 2026-09-07（彻底优化架构纪元：MINOR 逢十进 major v9.9.5→v10.0.0——三层护栏 L0/L1/L2 落地 + 记忆分片重构（MEMORY 7416→1840）+ automation memory 单源化（A2）+ sync_check 扩至 20 项门禁（⑧ 聚合/⑩ 元数据）+ 审校协议 P2 降级 note（Self-Critique Paradox）+ feed-learning 下沉测试门槛 + 周棘轮/issue-ledger + l3_publish ls-remote 判定（A6）；规则数不变仍 187=131+56；详见 CHANGELOG v10.0.0 条目）；基于 v9.9.5 | 2026-09-07（辛丑条约签订 L2：新增 Rule 131 多国联军侵华须区分侵华八国与签约十一国（P0，ima 1/4 指出 v1 混淆），186→187=131+56）；基于 v9.9.4 | 2026-09-05（Google成立 L2：新增 Rule 130 标题具象钩子正文兑现（ds+ima+豆包 3/4）+ Forbidden #56 无出处推测套话（ds+豆包 2/4），184→186=130+56）；基于 v9.8.13 | 2026-09-03（瑞典换边 Dagen H L2：新增 Rule 128 外文代号须点明并解释含义（ds+ima 2/4）+ Rule 129 反直觉好结果须补达成机理（ds+ima+豆包 3/4），182→184=129+55）；基于 v9.8.12 | 2026-09-02（清廷废科举 L2：新增 Rule 127 群体反应绝对化 + Forbidden #55 半截隐喻悬置，180→182=127+55）；基于 v9.8.11 | L1 自动化流程新增 Phase 2.5 标题复审（与 L2 对齐）；德国入侵波兰 L2（2026-09-01）：新增 R120 殖民时长精度/R121 宗主国动机多因/R122 去殖民武装抗争呼应（去殖民专项，均 2/4 共识），173→176（122+54）；基于 v9.8.8（第一架喷气式飞机首飞 L2：新增 Rule 118（正文避免插入面向读者的说教式过渡/纠正认知闲笔，P2，IMA+豆包 2/4 指出 v1 中段"今天的大多数旅客大概以为…"说教过渡打断节奏）+ Rule 119（同一人物/机构身份与归属表述须全文事实自洽，P1，DS+IMA+豆包 3/4 指出 v1 瓦西茨"空军部指派"与"绕开空军部"矛盾）+ Forbidden #54（克制无法验证的反事实英雄叙事断言，P2，豆包 1/4 独家指出 v1 "赌上私财绕过僵化军方体系"为反事实+主观断言），170→173 条（119+54）；基于 v9.8.7（喀拉喀托 L2 学习：新增 Rule 117（多强钩子标题双钩，P2，千问+豆包 2/4 指出原标题只点声音线），169→170 条（117+53）；巴黎解放 L2 学习：新增 Rule 115（受降/签字多地点流程区分，P0，ima+豆包 2/4 指出 v1 误将蒙帕纳斯车站列入受降签字地，实为戴高乐抵达地；正文附 writing_core.md）+ Rule 116（象征性细节主体存疑时模糊化，P2，豆包 1/4 独家指出埃菲尔铁塔升旗主体非消防队员、各源不一），167→169 条（116+53）；v2 吸收：受降流程修正（默里斯=投降地→警察总局=签字地，蒙帕纳斯改戴高乐抵达，P0）、补 8-19 起义规模与戴高乐政治博弈（ds+千问+豆包 3/4）、吸收"Brennt Paris?"题眼（ima 独家）、升旗主体模糊化（豆包独家）、补军事困境铺垫（豆包独家）、理顺时间线（豆包独家）、修复错字"让历史意外选择"缺"的"（ima 独家）；基于 v9.8.5（庞贝 L2 学习：新增 Rule 114，166→167，hot 68→69）；基于 v9.8.4（伽利略望远镜 L2：新增 Rule 113，165→166，hot 67→68）；基于 v9.8.3（布拉格之春 L2：新增 Rule 112 + Forbidden #53，163→165，hot 66→67）；基于 v9.8.2（达盖尔银版摄影法 L2：新增 Rule 111，162→163，hot 65→66）；基于 v9.8.1（妇女选举权 L2：Rule 108/109/110 + Forbidden #52，158→162，hot 62→65）；基于 v9.8.0（伊兹米特地震 L2：Rule 105/106/107，155→158）；基于 v9.7.9（全量审查修复：sync_check 崩溃/假阳性修复 + hot 正文完整性 + Rule97 去重 + 文档索引批量修正）；基于 v9.7.8（自动化卡顿优化：spawn 规范写死/熔断链/文件检测等待/冷规则瘦身/sync_check 一键核验/CASE_STUDIES 只 grep 禁整读/精修防死循环）；2026-08-26 增补 Phase 5b 选题索引（archive/daily/选题索引.md，两列：运行日期|选题名，数据源 TOPICS.md 选题复盘块，每次运行重建；Automation prompt Phase 6b 同步；版本号不变仍 v9.8.6）；基于 v9.8.9（马来亚独立 L2：新增 R120-122，规则 173→176）；2026-09-01 德国入侵波兰 L2：新增 Rule 123（落败方须见实际抵抗与装备对比，P1）+ Rule 124（冲击性事实点须具象细节或收紧措辞，P1）+ Rule 125（战术名词点明机械运作链条，P2）+ Rule 126（落败方决策须交代背景去事后评判腔，P2），176→180（126+54）*
