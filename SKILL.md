...
| **Phase 2-3 写作** | `writing_core.md` + `rule_index.md` +（按题材）`topics/` 1 个 +（按需）`craft_optional.md` +（按需）`fact_checklist.md` | 核心：叙事结构 + 6维工具包 + 写作标准 + 84条hot规则（core 61 + 题材专项 23，sync_check ⑤ 实跑口径）+ 温控表；索引：191条规则编号+摘要；题材专项×3；非强制技法；写作侧事实核查清单 | 核心~85K + 索引18K + 题材8-16K（+按需12K+4K） |
...
| **Phase 3.6 判例** | `review/CASE_STUDIES.md` | 68条案例（续号至68；只Grep命中关键词，禁止整读进上下文） | ~118K |
...
- 写完后必须运行 `scripts/sync_check.py` 核验（规则数 191=133+58、版本号、文件路径、hot 规则正文完整性）
...
| 写作核心 | `writing_core.md` | 叙事结构+6维工具包+84条hot规则（core 61 + 题材专项 23，sync_check ⑤ 实跑口径）+基础/hot禁止模式+温控表（v9.7.8瘦身：26条cold Forbidden正文与cold状态表已迁 archive） |
| 规则索引 | `rule_index.md` | 191条规则编号+摘要+温控+文件定位 |
...
| 判例库 | `review/CASE_STUDIES.md` | 68条案例（续号至68） |
