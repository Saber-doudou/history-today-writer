...
| **Phase 2-3 写作** | `writing_core.md` + `rule_index.md` +（按题材）`topics/` 1 个 +（按需）`craft_optional.md` +（按需）`fact_checklist.md` | 核心：叙事结构 + 6维工具包 + 写作标准 + 77条hot规则（core 55 + 题材专项 22，sync_check ⑤ 口径）+ 温控表；索引：180条规则编号+摘要；题材专项×3；非强制技法；写作侧事实核查清单 | 核心~85K + 索引18K + 题材8-16K（+按需12K+4K） |
...
| **Phase 3.6 判例** | `review/CASE_STUDIES.md` | 58条案例（续号至58；只Grep命中关键词，禁止整读进上下文） | ~118K |
...
Phase 2.5: 标题复审（强制，不可跳过，与 L2 feed-learning Phase 2.5 同款约束）
    ...
    - 与 Phase 4.5 ASO 分工：本步是写作端标题质量把关/重拟；Phase 4.5 是发布端标题+摘要+标签优化微调，不推翻核心钩子
    ↓
...
- 写完后必须运行 `scripts/sync_check.py` 核验（规则数 180=126+54、版本号、文件路径、hot 规则正文完整性）
...
| 写作核心 | `writing_core.md` | 叙事结构+6维工具包+77条hot规则（core 55 + 题材专项 22）+基础/hot禁止模式+温控表（v9.7.8瘦身：26条cold Forbidden正文与cold状态表已迁 archive） |
| 规则索引 | `rule_index.md` | 180条规则编号+摘要+温控+文件定位 |
...
| 判例库 | `review/CASE_STUDIES.md` | 62条案例（续号至62；CASE-61 存在近重复双块，待 Master 决策清理） |
